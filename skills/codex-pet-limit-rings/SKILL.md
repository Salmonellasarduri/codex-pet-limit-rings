...
Double-clicking the pet sends Codex's own `Cmd+Option+N` Quick Chat shortcut. Right-clicking uses the same shortcut first, then opens Codex Settings in the new tab. Do not implement this by mutating Codex's `active-workspace-roots`; a running Codex process keeps that state in memory.
